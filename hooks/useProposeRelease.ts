...
export type ProposeReleaseErrorCode =
...
export interface ProposeReleaseError {
  code: ProposeReleaseErrorCode
  message: string
}
    ...
    if (accessRes.error || !accessRes.address) {
      throw {
        ...
        message:
          ...
          "Autorizá este sitio en Freighter para crear la propuesta.",
      } satisfies ProposeReleaseError
    }
...
/**
 * propose_release(destination: address, amount: i128, group_id: u64, user: address) -> u64
 */
export function useProposeRelease() {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<ProposeReleaseError | null>(null)
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          setError(err as ProposeReleaseError)
          return null
        }
        const message =
