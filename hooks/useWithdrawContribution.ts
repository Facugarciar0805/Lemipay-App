...
function isWithdrawError(err: unknown): err is WithdrawError {
  return (
    typeof err === "object" &&
    err !== null &&
    "code" in err &&
    ...
    if (accessRes.error || !accessRes.address) {
      throw {
        ...
        message:
          ...
          "Autorizá este sitio en Freighter para retirar tu aporte.",
      } satisfies WithdrawError
    }
...
/**
 * Hook for withdrawing contribution from a fund round.
 * withdraw_contribution(round_id: u64, user: Address) -> Result<Void, Error>
 */
  ...
  const withdrawContribution = useCallback(
    async (roundId: bigint): Promise<string | null> => {
      ...
      try {
        ...
        if (isWithdrawError(err)) {
          setError(err)
          return null
        }
        const message =
          err instanceof Error ? err.message : "Ocurrió un error inesperado"
        const withdrawErr: WithdrawError = {
          code: "UNKNOWN",
          message,
        }
        setError(withdrawErr)
        return null
      } finally {
        setIsLoading(false)
      }
