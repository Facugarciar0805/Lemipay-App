...
export type ProposeFundRoundErrorCode =
...
export interface ProposeFundRoundError {
  code: ProposeFundRoundErrorCode
  message: string
}

function isProposeFundRoundError(err: unknown): err is ProposeFundRoundError {
  return (
    typeof err === "object" &&
    err !== null &&
    "code" in err &&
...
async function getPublicKey(): Promise<string> {
  if (typeof window === "undefined") {
    ...
    } satisfies ProposeFundRoundError
  }
    ...
    if (accessRes.error || !accessRes.address) {
      throw {
        ...
        message:
          ...
          "Autorizá este sitio en Freighter para proponer la ronda.",
      } satisfies ProposeFundRoundError
    }
...
/** USDC uses 7 decimals on-chain. 1 USDC = 10^7 units. */
const USDC_DECIMALS = 10_000_000

export function useProposeFundRound() {
  const [isLoading, setIsLoading] = useState(false)
  const [error, setError] = useState<ProposeFundRoundError | null>(null)
  const [txHash, setTxHash] = useState<string | null>(null)

  const proposeFundRound = useCallback(
    async (groupId: bigint, totalAmountUsdc: number): Promise<string | null> => {
      setIsLoading(true)
      setError(null)
      setTxHash(null)
      ...
      try {
        ...
        const totalAmountRaw = BigInt(
          Math.round(totalAmountUsdc * USDC_DECIMALS)
        )

        const invokeOp = contract.call(
          "propose_fund_round",
          nativeToScVal(groupId, { type: "u64" }),
          nativeToScVal(totalAmountRaw, { type: "i128" }),
        ...
        if (isProposeFundRoundError(err)) {
          setError(err)
          return null
        }
        const message =
          err instanceof Error ? err.message : "Ocurrió un error inesperado"
        const proposeErr: ProposeFundRoundError = {
          code: "UNKNOWN",
          message,
        }
        setError(proposeErr)
