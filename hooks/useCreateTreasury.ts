...
export type CreateTreasuryErrorCode =
...
export interface CreateTreasuryError {
  code: CreateTreasuryErrorCode
  message: string
}

function isCreateTreasuryError(err: unknown): err is CreateTreasuryError {
  return (
    typeof err === "object" &&
    err !== null &&
    "code" in err &&
    ...
    if (accessRes.error || !accessRes.address) {
      throw {
        ...
        message:
          ...
          "Autorizá este sitio en Freighter para crear la tesorería.",
      } satisfies CreateTreasuryError
      }
...
export function useCreateTreasury() {
  ...
  const [error, setError] = useState<CreateTreasuryError | null>(null)
  const [txHash, setTxHash] = useState<string | null>(null)

  const createTreasury = useCallback(
    async (groupId: bigint): Promise<string | null> => {
      setIsLoading(true)
      setError(null)
      setTxHash(null)
      ...
      try {
        ...
        if (isCreateTreasuryError(err)) {
          setError(err)
          return null
        }
        const message =
          err instanceof Error ? err.message : "Ocurrió un error inesperado"
        const createErr: CreateTreasuryError = {
          code: "UNKNOWN",
          message,
        }
        setError(createErr)
